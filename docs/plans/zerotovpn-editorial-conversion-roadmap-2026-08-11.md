...
- **12 August 2026 - supporting-article claim cleanup:** The compact production scan separated numeric speed values from unsupported provider-count marketing. Three genuine residual claims were found in the BBC iPlayer, Reddit torrenting and biometric-payment articles; their markdown and rendered JSON records now use current provider evidence, dated checks or explicit network-layer limits. Added six source/record guards; local editorial checks pass **36/36**, lint passes with 0 errors (53 existing warnings), and the full Webpack build still generates **4,447 routes**. Deployment and the post-deploy 22-page gate follow this release.
