...
| Partner conversions, revenue and EPC joined | Pending authenticated export | Nord affiliate dashboard currently resolves to login in the available session; no partner export is present, and the measurement guard correctly requires it before `ready: true` |
