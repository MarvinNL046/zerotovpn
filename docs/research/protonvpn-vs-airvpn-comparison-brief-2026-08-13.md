...
2. **Passed locally:** the English pair route now renders from an evidence-led component, not the legacy score/winner FAQ generator. Until promotion is approved, its metadata is explicitly `noindex, follow`; the browser smoke is recorded in `docs/metrics/proton-airvpn-comparison-browser-smoke-2026-08-13.md`. This does not yet add the route to the sitemap or production live gate.
