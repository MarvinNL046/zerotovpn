...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "brave-vpn");
const OUT = resolve(ROOT, "docs", "research");
const reportDate = new Date().toISOString().slice(0, 10);
const context = { ...DFS_DEFAULTS };
const seeds = [
  "is brave vpn free",
  "is brave firewall vpn free",
  "does brave have a free vpn",
  "how much is brave vpn",
  "brave vpn free trial",
  "brave vpn review",
];
...
async function main() {
  ...
  const report = {
    schemaVersion: 1,
    cluster: "Brave VPN pricing / free tier / browser integration",
    ...
    serp,
    sourcePolicy: "DataForSEO signals prioritise intent coverage and FAQ structure; they do not prove current pricing, trial eligibility, feature availability or provider performance.",
  };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-brave-vpn-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-brave-vpn-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = [
    "# DataForSEO research — Brave VPN free-tier and pricing intent", "",
    ...
    "Use these signals to distinguish the free Brave browser and Brave Shields from the separately offered Firewall + VPN product. Keep pricing, trial, platform and feature language bounded by current first-party terms and store availability; do not turn a SERP signal into a permanent offer claim. This dossier does not justify a new URL batch before matched partner measurement.", "",
  ];
