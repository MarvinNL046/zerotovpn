...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "chatgpt-vpn");
...
const seeds = [
  ...
  "vpn for chatgpt",
  "chatgpt vpn",
  "can a vpn access chatgpt",
  "chatgpt blocked in my country",
  "vpn for openai",
...
const cachePath = (endpoint, task) => resolve(
  CACHE,
  `${createHash("sha256").update(`${endpoint}:${stable(task)}`).digest("hex")}.json`,
);
...
function collectPaa(value, output = []) {
  ...
  if (Array.isArray(value)) {
    value.forEach((item) => collectPaa(item, output));
    return output;
  }
...
async function main() {
  ...
  for (const seed of seeds) {
    const payload = await cached("dataforseo_labs/google/keyword_suggestions/live", {
      ...context,
      keyword: seed,
      limit: 15,
    ...
    for (const item of items(payload)) {
      ...
      if (keyword) suggestions.push({
        seed,
        keyword,
        volume: item.keyword_info?.search_volume ?? item.keyword_data?.keyword_info?.search_volume ?? null,
      });
    }
  }
  const serp = [];
  for (const keyword of seeds) {
    try {
      ...
      serp.push({
        keyword,
        paa: [...new Set(collectPaa(payload.result))].slice(0, 12),
        hasAiOverview: JSON.stringify(payload.result).includes('"type":"ai_overview"'),
      });
    } catch (error) {
      serp.push({ keyword, paa: [], hasAiOverview: false, error: error instanceof Error ? error.message : String(error) });
    }
  }

  const report = {
    schemaVersion: 1,
    cluster: "ChatGPT access, availability, VPN network privacy and account limits",
    ...
    serp,
    sourcePolicy: "DataForSEO signals prioritise access-intent and FAQ coverage; they do not prove current country availability, legality, OpenAI policy, provider performance or conversion value.",
  };

  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-chatgpt-vpn-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-chatgpt-vpn-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = [
    "# DataForSEO research — ChatGPT VPN cluster",
    "",
    ...
    ...serp.flatMap((row) => [
      `### ${row.keyword}`,
      `- AI Overview: ${row.hasAiOverview ? "yes" : "no"}`,
      ...(row.error ? [`- SERP request note: ${row.error}`] : []),
      ...(row.paa.length ? row.paa.map((question) => `- PAA: ${question}`) : ["- No PAA returned"]),
      "",
    ]),
    ...
    "| Seed | Keyword | Volume |",
    "|---|---|---:|",
    ...report.suggestions.slice(0, 120).map((row) => `| ${row.seed} | ${row.keyword} | ${row.volume ?? "n/a"} |`),
    "",
    "## Editorial interpretation",
    "",
    "Refresh the existing commercial page around current availability checks, network-versus-platform privacy boundaries, account and location limits, lawful-use caveats, device setup and provider verification. Keep country availability and provider claims tied to first-party sources and do not turn PAA wording into an access guarantee.",
    "",
  ];
