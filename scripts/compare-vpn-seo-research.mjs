...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "vpn-comparison");
const OUT = resolve(ROOT, "docs", "research");
const reportDate = new Date().toISOString().slice(0, 10);
const context = { ...DFS_DEFAULTS };
const seeds = ["vpn comparison", "compare vpn services", "best vpn comparison", "vpn comparison 2026", "nordvpn vs expressvpn", "vpn price comparison"];
...
async function main() {
  ...
  const suggestionPayloads = await Promise.all(seeds.map((keyword) => cached("dataforseo_labs/google/keyword_suggestions/live", { ...context, keyword, limit: 15, include_seed_keyword: true })));
  ...
  const serpSeeds = seeds.slice(0, 4);
  ...
  const report = { schemaVersion: 1, cluster: "VPN comparison / price / provider selection", targetDomain: "zerotovpn.com", ...context, fetchedAt: new Date().toISOString(), refreshed: process.argv.includes("--refresh"), seeds, overview, suggestions: [...new Map(suggestions.map((row) => [row.keyword, row])).values()], serp, sourcePolicy: "DataForSEO signals prioritise comparison intent and question coverage; they do not prove provider performance, pricing, rankings or conversion value. Provider facts must be tied to dated catalog or first-party evidence." };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-vpn-comparison-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-vpn-comparison-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = ["# DataForSEO research — VPN comparison cluster", "", `Fetched: ${report.fetchedAt} | location ${report.location_code} | language ${report.language_code}`, "", "## Keyword overview", "", "| Keyword | Current | Latest non-zero | KD | Intent |", "|---|---:|---:|---:|---|", ...overview.map((row) => `| ${row.keyword} | ${row.currentVolume ?? "n/a"} | ${row.latestVolume ?? "n/a"}${row.latestMonth ? ` (${row.latestMonth})` : ""} | ${row.difficulty ?? "n/a"} | ${row.intent ?? "n/a"} |`), "", "## PAA and SERP features", "", ...serp.flatMap((row) => [`### ${row.keyword}`, `- AI Overview: ${row.hasAiOverview ? "yes" : "no"}`, ...(row.paa.length ? row.paa.map((question) => `- PAA: ${question}`) : ["- No PAA returned"]), ""]), "## Suggestion candidates", "", "| Seed | Keyword | Volume |", "|---|---|---:|", ...report.suggestions.slice(0, 120).map((row) => `| ${row.seed} | ${row.keyword} | ${row.volume ?? "n/a"} |`), "", "## Editorial interpretation", "", "Build the comparison around transparent decision criteria: price snapshot and renewal, refund window, simultaneous-device wording, protocol/app support and privacy evidence. Avoid universal fastest/best labels unless a dated, reproducible test record supports them.", ""];
