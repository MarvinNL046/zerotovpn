...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "connection-drops");
...
const seeds = [
  ...
  "how to fix vpn connection drops",
  "vpn disconnects when switching wifi",
  "vpn kill switch disconnects internet",
];
...
async function main() {
  ...
  const report = {
    schemaVersion: 1,
    cluster: "VPN connection drops / troubleshooting / network switching",
    ...
    serp,
    sourcePolicy: "DataForSEO signals prioritise intent coverage and FAQ structure; they do not prove provider stability, fix success, security outcomes, pricing or conversion value.",
  };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-connection-drops-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-connection-drops-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = [
    "# DataForSEO research — VPN connection drops cluster",
    "",
    ...
    "",
    "Use this dossier to clarify diagnosis and recovery steps on the existing article. Keep network, protocol and kill-switch behaviour bounded by the user's device, ISP and provider documentation. Do not turn PAA wording into provider guarantees or a new URL batch before the matched partner measurement is available.",
    "",
  ];
