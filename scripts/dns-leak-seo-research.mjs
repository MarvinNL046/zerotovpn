...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "dns-leak");
...
const overviewRows = (payload) => items(payload).map((item) => {
...
async function main() {
  ...
  const suggestions = [];
  for (const seed of seeds) for (const item of items(await cached("dataforseo_labs/google/keyword_suggestions/live", { ...context, keyword: seed, limit: 15, include_seed_keyword: true }))) {
  ...
  const report = { schemaVersion: 1, cluster: "DNS leak testing / VPN privacy boundaries", targetDomain: "zerotovpn.com", ...context, fetchedAt: new Date().toISOString(), refreshed: process.argv.includes("--refresh"), seeds, overview, suggestions: [...new Map(suggestions.map((row) => [row.keyword, row])).values()], serp, sourcePolicy: "DataForSEO signals prioritise test intent, diagnosis questions and FAQ coverage; they do not prove that a VPN prevents every DNS leak or that a provider's current implementation works on every device." };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-dns-leak-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-dns-leak-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = ["# DataForSEO research — DNS leak testing", "", `Fetched: ${report.fetchedAt} | location ${report.location_code} | language ${report.language_code}`, "", "## Keyword overview", "", "| Keyword | Current | Latest non-zero | KD | Intent |", "|---|---:|---:|---:|---|", ...overview.map((row) => `| ${row.keyword} | ${row.currentVolume ?? "n/a"} | ${row.latestVolume ?? "n/a"}${row.latestMonth ? ` (${row.latestMonth})` : ""} | ${row.difficulty ?? "n/a"} | ${row.intent ?? "n/a"} |`), "", "## PAA and SERP features", "", ...serp.flatMap((row) => [`### ${row.keyword}`, `- AI Overview: ${row.hasAiOverview ? "yes" : "no"}`, ...(row.paa.length ? row.paa.map((question) => `- PAA: ${question}`) : ["- No PAA returned"]), ""]), "## Suggestion candidates", "", "| Seed | Keyword | Volume |", "|---|---|---:|", ...report.suggestions.slice(0, 120).map((row) => `| ${row.seed} | ${row.keyword} | ${row.volume ?? "n/a"} |`), "", "## Editorial interpretation", "", "Make the tool the primary action, then explain what a DNS result can and cannot establish. Separate ISP-DNS exposure, browser DoH/DoT, IPv6 and VPN-client routing; use provider pages only as dated implementation evidence.", ""];
