...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "fitness-tracking-privacy");
...
const seeds = [
  ...
  "garmin connect privacy",
  "vpn for fitness tracking",
];
...
async function main() {
  ...
  const report = {
    schemaVersion: 1,
    cluster: "Fitness tracking privacy / GPS data / wearable app boundaries",
    ...
    serp,
    sourcePolicy: "DataForSEO signals prioritise intent coverage and FAQ structure; they do not prove an app's current data practices, legal compliance or that a VPN can block GPS collection.",
  };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-fitness-tracking-privacy-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-fitness-tracking-privacy-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = [
    "# DataForSEO research — fitness tracking privacy", "",
    `Fetched: ${report.fetchedAt} | location ${report.location_code} | language ${report.language_code}`, "",
    "## Keyword overview", "", "| Keyword | Current | Latest non-zero | KD | Intent |", "|---|---:|---:|---:|---|",
    ...
    ...serp.flatMap((row) => [`### ${row.keyword}`, `- AI Overview: ${row.hasAiOverview ? "yes" : "no"}`, ...(row.error ? [`- SERP request note: ${row.error}`] : []), ...(row.paa.length ? row.paa.map((question) => `- PAA: ${question}`) : ["- No PAA returned"]), ""]),
    "## Suggestion candidates", "", "| Seed | Keyword | Volume |", "|---|---|---:|",
    ...report.suggestions.slice(0, 110).map((row) => `| ${row.seed} | ${row.keyword} | ${row.volume ?? "n/a"} |`), "",
    "## Editorial interpretation", "",
    "Use these signals to explain the boundary between IP/network privacy and GPS or app-level collection. Tie every app-policy claim to a dated first-party source; do not turn PAA wording into a guarantee, medical statement or legal conclusion.", "",
  ];
