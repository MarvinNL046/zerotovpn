...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "gaming");
...
async function main() {
  ...
  const report = {
    schemaVersion: 1,
    cluster: "VPN for gaming / latency / DDoS / console setup",
    ...
    serp,
    sourcePolicy: "DataForSEO signals prioritise intent coverage and FAQ structure; they do not prove current provider latency, DDoS protection, console support, pricing or conversion value.",
  };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-gaming-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-gaming-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = [
    "# DataForSEO research — Gaming VPN cluster",
    "",
    ...
    "",
    "Use the signals to clarify gaming intent on the existing page: latency is network-dependent, DDoS protection is an architecture and threat-model question, and console support varies by router or Smart DNS setup. Keep provider facts, prices and performance claims tied to dated first-party or reproducible evidence. This dossier does not justify a new URL batch before the matched partner measurement is available.",
    "",
  ];
