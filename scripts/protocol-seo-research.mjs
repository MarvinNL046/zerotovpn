...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "protocols");
...
const seeds = [
  ...
  "best vpn protocol",
  "openvpn tcp vs udp",
  "vpn obfuscation protocol",
  "stealth vpn",
  "vpn protocol for china",
];
...
async function main() {
  ...
  const report = { schemaVersion: 1, cluster: "VPN protocols / obfuscation / WireGuard vs OpenVPN", targetDomain: "zerotovpn.com", ...context, fetchedAt: new Date().toISOString(), refreshed: process.argv.includes("--refresh"), seeds, overview: overviewRows, suggestions: [...new Map(suggestions.map((row) => [row.keyword, row])).values()], serp, competitors, sourcePolicy: "DataForSEO signals prioritise questions and structure; they do not prove a protocol is secure, available, lawful or effective on a particular network." };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, "dataforseo-protocols-cluster-2026-08-11.json");
  const mdPath = resolve(OUT, "dataforseo-protocols-cluster-2026-08-11.md");
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = ["# DataForSEO research - VPN protocols and obfuscation", "", `Fetched: ${report.fetchedAt} | location ${report.location_code} | language ${report.language_code}`, "", "## Keyword overview", "", "| Keyword | Current | Latest non-zero | KD | Intent |", "|---|---:|---:|---:|---|", ...overviewRows.map((row) => `| ${row.keyword} | ${row.currentVolume ?? "n/a"} | ${row.latestVolume ?? "n/a"}${row.latestMonth ? ` (${row.latestMonth})` : ""} | ${row.difficulty ?? "n/a"} | ${row.intent ?? "n/a"} |`), "", "## PAA and SERP features", "", ...serp.flatMap((row) => [`### ${row.keyword}`, `- AI Overview: ${row.hasAiOverview ? "yes" : "no"}`, ...(row.paa.length ? row.paa.map((question) => `- PAA: ${question}`) : ["- No PAA returned"]), ""]), "## Suggestion candidates", "", "| Seed | Keyword | Volume |", "|---|---|---:|", ...report.suggestions.slice(0, 80).map((row) => `| ${row.seed} | ${row.keyword} | ${row.volume ?? "n/a"} |`), "", "## Competitors", "", ...competitors.map((domain) => `- ${domain}`), "", "## Editorial interpretation", "", "Use current primary protocol documentation for technical claims. Treat provider feature pages as feature evidence, not access guarantees. Keep restrictive-network advice lawful and bounded."];
