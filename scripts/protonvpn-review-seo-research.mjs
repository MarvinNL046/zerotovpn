...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "protonvpn-review");
const OUT = resolve(ROOT, "docs", "research");
const reportDate = new Date().toISOString().slice(0, 10);
const context = { ...DFS_DEFAULTS };
const seeds = ["proton vpn review", "proton vpn review 2026", "proton vpn free plan", "proton vpn no logs", "proton vpn speed test", "is proton vpn worth it", "proton vpn streaming", "proton vpn vs nordvpn"];
...
async function main() {
  ...
  const report = { schemaVersion: 1, cluster: "Proton VPN review / free plan / privacy / speed / streaming", targetDomain: "zerotovpn.com", ...context, fetchedAt: new Date().toISOString(), refreshed: process.argv.includes("--refresh"), seeds, overview, suggestions: [...new Map(suggestions.map((row) => [row.keyword, row])).values()], serp, sourcePolicy: "DataForSEO signals prioritise query intent, comparison structure and FAQ coverage; they do not prove Proton VPN performance, current pricing, audit scope or conversion value. Provider facts must be tied to dated first-party sources or reproducible tests." };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-protonvpn-review-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-protonvpn-review-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = ["# DataForSEO research — Proton VPN review cluster", "", `Fetched: ${report.fetchedAt} | location ${report.location_code} | language ${report.language_code}`, "", "## Keyword overview", "", "| Keyword | Current | Latest non-zero | KD | Intent |", "|---|---:|---:|---:|---|", ...overview.map((row) => `| ${row.keyword} | ${row.currentVolume ?? "n/a"} | ${row.latestVolume ?? "n/a"}${row.latestMonth ? ` (${row.latestMonth})` : ""} | ${row.difficulty ?? "n/a"} | ${row.intent ?? "n/a"} |`), "", "## PAA and SERP features", "", ...serp.flatMap((row) => [`### ${row.keyword}`, `- AI Overview: ${row.hasAiOverview ? "yes" : "no"}`, ...(row.paa.length ? row.paa.map((question) => `- PAA: ${question}`) : ["- No PAA returned"]), ""]), "## Suggestion candidates", "", "| Seed | Keyword | Volume |", "|---|---|---:|", ...report.suggestions.slice(0, 140).map((row) => `| ${row.seed} | ${row.keyword} | ${row.volume ?? "n/a"} |`), "", "## Editorial interpretation", "", "Use one evidence-led review to answer plan, free-tier, privacy, speed and streaming questions. Keep claims dated and bounded; tracked affiliate links remain in relevant VPN-selection contexts with clear disclosure.", ""];
