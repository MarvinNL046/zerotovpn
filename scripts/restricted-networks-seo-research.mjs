...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "restricted-networks");
const OUT = resolve(ROOT, "docs", "research");
const context = { ...DFS_DEFAULTS };
const seeds = [
  "vpn for restricted networks",
  "how to use a vpn on a restricted network",
  "vpn censorship",
  "bypass internet censorship vpn",
  "vpn for censored internet",
  "vpn restrictions at work",
];
...
async function main() {
  ...
  const serp = [];
  for (const keyword of seeds.slice(0, 5)) { const payload = await cached("serp/google/organic/live/advanced", { ...context, keyword, depth: 20 }); serp.push({ keyword, paa: [...new Set(paa(payload.result))].slice(0, 12), hasAiOverview: JSON.stringify(payload.result).includes('"type":"ai_overview"') }); }
  const report = { schemaVersion: 1, cluster: "VPN use on restricted networks / censorship preparation", targetDomain: "zerotovpn.com", ...context, fetchedAt: new Date().toISOString(), refreshed: process.argv.includes("--refresh"), seeds, overview: overviewRows, suggestions: [...new Map(suggestions.map((row) => [row.keyword, row])).values()], serp, sourcePolicy: "DataForSEO signals prioritise questions and structure; they do not prove that bypassing a restriction is safe, lawful or effective on a particular network." };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, "dataforseo-restricted-networks-cluster-2026-08-11.json");
  const mdPath = resolve(OUT, "dataforseo-restricted-networks-cluster-2026-08-11.md");
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = ["# DataForSEO research - restricted networks and censorship preparation", "", `Fetched: ${report.fetchedAt} | location ${report.location_code} | language ${report.language_code}`, "", "## Keyword overview", "", "| Keyword | Current | Latest non-zero | KD | Intent |", "|---|---:|---:|---:|---|", ...overviewRows.map((row) => `| ${row.keyword} | ${row.currentVolume ?? "n/a"} | ${row.latestVolume ?? "n/a"}${row.latestMonth ? ` (${row.latestMonth})` : ""} | ${row.difficulty ?? "n/a"} | ${row.intent ?? "n/a"} |`), "", "## PAA and SERP features", "", ...serp.flatMap((row) => [`### ${row.keyword}`, `- AI Overview: ${row.hasAiOverview ? "yes" : "no"}`, ...(row.paa.length ? row.paa.map((question) => `- PAA: ${question}`) : ["- No PAA returned"]), ""]), "## Suggestion candidates", "", "| Seed | Keyword | Volume |", "|---|---|---:|", ...report.suggestions.slice(0, 80).map((row) => `| ${row.seed} | ${row.keyword} | ${row.volume ?? "n/a"} |`), "", "## Editorial interpretation", "", "Use dated primary sources for legal and network claims. Keep advice lawful, bounded and preparation-focused; do not promise permanent access."];
