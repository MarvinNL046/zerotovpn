...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "server-location");
const OUT = resolve(ROOT, "docs", "research");
const reportDate = new Date().toISOString().slice(0, 10);
const context = { ...DFS_DEFAULTS };
const seeds = [
  "best country for vpn server",
  "best vpn server location",
  "fastest vpn server location",
  "best country for vpn privacy",
  "which vpn server should i use",
  "vpn server location for streaming",
];
...
async function main() {
  ...
  const report = {
    schemaVersion: 1,
    cluster: "VPN server location / speed / privacy / streaming choice",
    ...
    serp,
    sourcePolicy: "DataForSEO signals prioritise intent coverage and FAQ structure; they do not prove a country is fastest, safest, legal or able to unlock a particular service.",
  };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-server-location-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-server-location-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = [
    "# DataForSEO research — VPN server location cluster", "",
    ...
    "Use these signals to explain how distance, route, jurisdiction, server availability and use case affect a location choice. Keep speed, privacy, legal and streaming language bounded by dated provider or official evidence. This dossier does not justify a new URL batch before matched partner measurement.", "",
  ];
