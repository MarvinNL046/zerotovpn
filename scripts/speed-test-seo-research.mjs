...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "speed-test");
...
async function main() {
  ...
  const report = {
    schemaVersion: 1,
    cluster: "Internet and VPN speed testing / latency / measurement interpretation",
    ...
    serp,
    sourcePolicy: "DataForSEO signals prioritise measurement questions and FAQ structure; they do not prove a speed-test result, ISP quality, VPN performance or percentage speed loss.",
  };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-speed-test-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-speed-test-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = [
    "# DataForSEO research — Speed test cluster",
    "",
    ...
    "",
    "Keep the existing speed test as an interactive measurement tool, but explain that a client-to-Cloudflare result is a route-specific observation rather than a universal ISP or VPN ranking. Separate throughput, latency, jitter and packet loss; compare direct and VPN runs under the same conditions and avoid fixed percentage-loss promises.",
    "",
  ];
