...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "surfshark-review");
const OUT = resolve(ROOT, "docs", "research");
const reportDate = new Date().toISOString().slice(0, 10);
const context = { ...DFS_DEFAULTS };
const seeds = ["surfshark review", "surfshark review 2026", "surfshark no logs", "surfshark unlimited devices", "surfshark price", "is surfshark worth it", "surfshark speed test", "surfshark netflix"];
...
async function main() {
  ...
  const report = { schemaVersion: 1, cluster: "Surfshark review / privacy / unlimited devices / price / streaming", targetDomain: "zerotovpn.com", ...context, fetchedAt: new Date().toISOString(), refreshed: process.argv.includes("--refresh"), seeds, overview, suggestions: [...new Map(suggestions.map((row) => [row.keyword, row])).values()], serp, sourcePolicy: "DataForSEO signals prioritise query intent, comparison structure and FAQ coverage; they do not prove Surfshark performance, current pricing, assurance scope or conversion value. Provider facts must be tied to dated first-party sources or reproducible tests." };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-surfshark-review-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-surfshark-review-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = ["# DataForSEO research — Surfshark review cluster", "", `Fetched: ${report.fetchedAt} | location ${report.location_code} | language ${report.language_code}`, "", "## Keyword overview", "", "| Keyword | Current | Latest non-zero | KD | Intent |", "|---|---:|---:|---:|---|", ...overview.map((row) => `| ${row.keyword} | ${row.currentVolume ?? "n/a"} | ${row.latestVolume ?? "n/a"}${row.latestMonth ? ` (${row.latestMonth})` : ""} | ${row.difficulty ?? "n/a"} | ${row.intent ?? "n/a"} |`), "", "## PAA and SERP features", "", ...serp.flatMap((row) => [`### ${row.keyword}`, `- AI Overview: ${row.hasAiOverview ? "yes" : "no"}`, ...(row.paa.length ? row.paa.map((question) => `- PAA: ${question}`) : ["- No PAA returned"]), ""]), "## Suggestion candidates", "", "| Seed | Keyword | Volume |", "|---|---|---:|", ...report.suggestions.slice(0, 140).map((row) => `| ${row.seed} | ${row.keyword} | ${row.volume ?? "n/a"} |`), "", "## Editorial interpretation", "", "Use one evidence-led review to answer plan economics, renewal, unlimited devices, privacy assurance, performance and streaming questions. Keep current terms dated and provider claims bounded.", ""];
