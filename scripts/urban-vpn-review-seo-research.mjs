...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "urban-vpn-review");
const OUT = resolve(ROOT, "docs", "research");
const reportDate = new Date().toISOString().slice(0, 10);
const context = { ...DFS_DEFAULTS };
const seeds = ["urban vpn review", "urban vpn review 2026", "is urban vpn safe", "urban vpn free", "urban vpn privacy", "urban vpn ai chats", "urban vpn vs proton vpn"];
...
async function main() {
  ...
  const report = { schemaVersion: 1, cluster: "Urban VPN review / free plan / privacy / browser-extension reporting", targetDomain: "zerotovpn.com", ...context, fetchedAt: new Date().toISOString(), refreshed: process.argv.includes("--refresh"), seeds, overview, suggestions: [...new Map(suggestions.map((row) => [row.keyword, row])).values()], serp, sourcePolicy: "DataForSEO signals prioritise query intent, comparison structure and FAQ coverage; they do not prove Urban VPN safety, current pricing, incident scope or conversion value. Provider facts must be tied to current first-party terms, dated reporting or reproducible tests." };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-urban-vpn-review-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-urban-vpn-review-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = ["# DataForSEO research — Urban VPN review cluster", "", `Fetched: ${report.fetchedAt} | location ${report.location_code} | language ${report.language_code}`, "", "## Keyword overview", "", "| Keyword | Current | Latest non-zero | KD | Intent |", "|---|---:|---:|---:|---|", ...overview.map((row) => `| ${row.keyword} | ${row.currentVolume ?? "n/a"} | ${row.latestVolume ?? "n/a"}${row.latestMonth ? ` (${row.latestMonth})` : ""} | ${row.difficulty ?? "n/a"} | ${row.intent ?? "n/a"} |`), "", "## PAA and SERP features", "", ...serp.flatMap((row) => [`### ${row.keyword}`, `- AI Overview: ${row.hasAiOverview ? "yes" : "no"}`, ...(row.paa.length ? row.paa.map((question) => `- PAA: ${question}`) : ["- No PAA returned"]), ""]), "## Suggestion candidates", "", "| Seed | Keyword | Volume |", "|---|---|---:|", ...report.suggestions.slice(0, 140).map((row) => `| ${row.seed} | ${row.keyword} | ${row.volume ?? "n/a"} |`), "", "## Editorial interpretation", "", "Use the signals to structure one evidence-led review around free-plan trade-offs, privacy-policy questions, network model, extension reporting and safer alternatives. Keep dated incident claims bounded and do not treat free pricing as safety evidence.", ""];
