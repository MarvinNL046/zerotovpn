...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "vpn-account-sharing");
const OUT = resolve(ROOT, "docs", "research");
const reportDate = new Date().toISOString().slice(0, 10);
const context = { ...DFS_DEFAULTS };
const seeds = [
  "can you share a vpn account",
  "vpn account sharing rules",
  "is it legal to share a vpn subscription",
  "how many devices can use a vpn",
...
async function main() {
  ...
  const report = {
    schemaVersion: 1,
    cluster: "VPN account sharing / simultaneous connections / provider terms",
    ...
    serp,
    sourcePolicy: "DataForSEO signals prioritise intent coverage and FAQ structure; they do not prove that sharing is permitted by any specific provider, or establish legal advice.",
  };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-vpn-account-sharing-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-vpn-account-sharing-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = [
    "# DataForSEO research — VPN account-sharing intent", "",
    ...
    "Use these signals to distinguish provider-defined simultaneous-device limits from sharing credentials with another person. Keep policy language bounded by current provider terms and avoid legal conclusions. This dossier does not justify a new URL batch before matched partner measurement.", "",
  ];
