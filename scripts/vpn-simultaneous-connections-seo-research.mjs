...
const CACHE = resolve(ROOT, ".cache", "dataforseo", "vpn-simultaneous-connections");
...
const seeds = [
  ...
  "vpn device limit",
  "can i use a vpn on two devices",
...
async function main() {
  ...
  const report = {
    schemaVersion: 1,
    cluster: "VPN simultaneous connections / device limits / household planning",
    ...
    serp,
    sourcePolicy: "DataForSEO signals prioritise intent coverage and FAQ structure; they do not prove a provider's current device policy, sharing permission or legal advice.",
  };
  mkdirSync(OUT, { recursive: true });
  const jsonPath = resolve(OUT, `dataforseo-vpn-simultaneous-connections-cluster-${reportDate}.json`);
  const mdPath = resolve(OUT, `dataforseo-vpn-simultaneous-connections-cluster-${reportDate}.md`);
  writeFileSync(jsonPath, JSON.stringify(report, null, 2));
  const lines = [
    "# DataForSEO research — VPN simultaneous connections", "",
    ...
    "Use these signals to explain the difference between installed devices, simultaneous connections and provider-defined household rules. Keep numbers and permission language tied to a dated provider source; do not turn PAA wording into a guarantee or legal conclusion.", "",
  ];
