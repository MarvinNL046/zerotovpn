...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: generateAlternates("/best/vpn-gaming", locale),
  };
}
                ...
                item.vpn ? (
                  <VpnFeatureCard
                    ...
                    badgeIcon={
                      ...
                      {item.badgeColor === "blue" && <Gamepad2 className="mr-1 size-3" aria-hidden="true" />}
                      {item.badgeColor === "green" && <Target className="mr-1 size-3" aria-hidden="true" />}
                      {item.badgeColor === "purple" && <Server className="mr-1 size-3" aria-hidden="true" />}
                      </>
                    }
                    rows={[
                    { label: "Protocol:", value: item.protocol },
                    { label: "Avg Ping:", value: item.avgPing, mono: true, valueClassName: "text-green-600 dark:text-green-500" },
                    { label: "Devices:", value: item.devices, mono: true },
                    { label: "Countries:", value: item.countries, mono: true },
                    ]}
