...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const titles: Record<string, string> = {
    en: `Best VPNs for Linux (${shortMonthYear}) — Native Apps & CLI | ZeroToVPN`,
                ...
                item.vpn ? (
                  <VpnFeatureCard
                    ...
                    badgeIcon={
                      ...
                      {item.badgeColor === "blue" && <Terminal className="mr-1 size-3" aria-hidden="true" />}
                      {item.badgeColor === "green" && <Target className="mr-1 size-3" aria-hidden="true" />}
                      </>
                    }
                    rows={[
                    { label: "CLI:", value: item.cliSupport },
                    { label: "Protocol:", value: item.protocol },
                    { label: "Distros:", value: item.distros, valueClassName: "text-xs" },
                    ]}
