...
import { VpnSpeedGuidePage } from "@/components/guides/vpn-speed-guide";
import {
  getVpnSpeedGuideCopy,
  isVpnSpeedGuideLocale,
} from "@/data/vpn-speed-guide";
import { BASE_URL, OG_LOCALE_MAP, titelMetMerk } from "@/lib/seo-utils";

type Props = { params: Promise<{ locale: string }> };

const PATH = "/guides/vpn-speed-guide";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const localized = isVpnSpeedGuideLocale(locale);
  const selectedLocale = localized ? locale : "en";
  const copy = getVpnSpeedGuideCopy(selectedLocale);
...
export default async function VpnSpeedGuideRoute({ params }: Props) {
  const { locale } = await params;
  if (!isVpnSpeedGuideLocale(locale)) permanentRedirect(`/en${PATH}`);
