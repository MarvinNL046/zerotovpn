...
import { MethodologyPageView } from "@/components/trust/methodology-page";
import {
  getMethodologyPageCopy,
  isMethodologyLocale,
} from "@/data/methodology-page";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const localized = isMethodologyLocale(locale);
...
export default async function MethodologyPage({ params }: Props) {
  const { locale } = await params;
  if (!isMethodologyLocale(locale)) permanentRedirect(`/en${PATH}`);
