...
import { PrivacyCookiePolicyPage } from "@/components/legal/privacy-cookie-policy-page";
import {
  getPrivacyPolicyCopy,
  isPolicyLocale,
} from "@/data/privacy-cookie-policy";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const localized = isPolicyLocale(locale);
  const contentLocale = localized ? locale : "en";
  const copy = getPrivacyPolicyCopy(contentLocale);
...
export default async function PrivacyPolicyPage({ params }: Props) {
  const { locale } = await params;
  if (!isPolicyLocale(locale)) permanentRedirect(`/en${PATH}`);
  ...
  return (
    <>
      ...
      />
      <PrivacyCookiePolicyPage copy={copy} />
    </>
  );
}
