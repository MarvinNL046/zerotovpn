...
import { TermsPageView } from "@/components/trust/terms-page";
import { getTermsPageCopy, isTermsLocale } from "@/data/terms-page";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const localized = isTermsLocale(locale);
  const selectedLocale = localized ? locale : "en";
  const copy = getTermsPageCopy(selectedLocale);
...
export default async function TermsPage({ params }: Props) {
  const { locale } = await params;
  if (!isTermsLocale(locale)) permanentRedirect(`/en${PATH}`);
