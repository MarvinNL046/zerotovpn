// Vercel API route for blog generation (ported from Netlify Background Function).
// Returns 200 immediately (fire-and-forget pattern not needed since Vercel Pro
...
// Scrape country data from multiple sources
async function scrapeCountryDataForBlog(country: CountryInfo): Promise<string | null> {
...
async function generateAI(prompt: string, model: AiModel): Promise<string> {
  return generateContent(prompt, { model, maxTokens: 16384, temperature: 0.7 });
}
...
// Kept inline (vs importing from image-generator.ts) because the lib version throws on failure,
  ...
  try {
    ...
    if (detectedCountry) {
      ...
      if (vpnSlugsInTopic.length > 0) {
        ...
        if (newsData.length > 0) contextParts.push(`RECENT VPN NEWS:\n${newsData.join("\n").slice(0, 2000)}`);
        if (pricingData.length > 0) contextParts.push(`VPN PRICING DATA (use these exact prices):\n${pricingData.join("\n\n").slice(0, 2000)}`);
        scrapeContext = contextParts.join("\n\n") || recentScrapes.map((s) => s.result).filter(Boolean).join("\n\n").slice(0, 4000);
      }
    }
    ...
    // Mark job as completed BEFORE translations (so the polling client doesn't time out)
    if (jobId) {
      await db
        .update(contentQueue)
        .set({
