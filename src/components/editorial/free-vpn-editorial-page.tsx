...
import { freeVpnCopy, type FreeVpnCardCopy, type FreeVpnCopy } from "@/components/editorial/free-vpn-copy";
...
export function FreeVpnEditorialPage({ locale = "en" }: { locale?: string } = {}) {
  const copy = freeVpnCopy[locale === "fr" ? "fr" : "en"];
  ...
  return (
    <>
      ...
      <BreadcrumbSchema items={[{ name: locale === "fr" ? "Meilleurs VPN" : "Best VPNs", href: "/best/best-vpn" }, { name: locale === "fr" ? "VPN gratuits" : "Free VPNs", href: "/best/free-vpn" }]} />
      ...
      <BestVpnEditorialTemplate brief={editorialContentBriefs.freeVpn} navigation={copy.nav}>
        <article>
          <section className="border-b bg-gradient-to-br from-emerald-500/10 via-background to-background py-14 lg:py-20"><div className="container max-w-5xl"><div className="max-w-3xl"><p className="text-sm font-semibold uppercase tracking-[0.16em] text-primary">{copy.updated}</p><h1 className="mt-4 text-4xl font-bold tracking-tight sm:text-6xl">{copy.heroTitle}</h1><p className="mt-6 text-xl leading-8 text-muted-foreground">{copy.heroIntro}</p><p className="mt-5 text-sm leading-6 text-muted-foreground">{copy.disclosure} <Link href="/affiliate-disclosure" className="underline">{locale === "fr" ? "lire notre déclaration" : "read our disclosure"}</Link>. {locale === "fr" ? "Les forfaits et politiques des fournisseurs peuvent changer." : "Provider plans and policies can change."}</p></div></div></section>
          ...
          <section id="paid-upgrade" className="container max-w-5xl scroll-mt-24 py-12 lg:py-16"><div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-primary">{copy.paidLabel}</p><h2 className="mt-3 text-3xl font-bold">{copy.paidTitle}</h2><p className="mt-4 leading-7 text-muted-foreground">{copy.paidIntro}</p></div><div className="rounded-xl border border-amber-300 bg-amber-50 p-5 text-sm leading-6 text-amber-950"><div className="flex gap-3"><TriangleAlert className="mt-0.5 size-5 shrink-0" aria-hidden="true" /><p><strong>{copy.restrictedLead}</strong> {copy.restrictedBody} <Link href="/guides/vpn-for-restricted-networks" className="underline">{locale === "fr" ? "guide des réseaux restreints" : "restricted-network guide"}</Link>, <Link href="/guides/vpn-obfuscation-explained" className="underline">{locale === "fr" ? "guide de l’obfuscation" : "obfuscation guide"}</Link> {locale === "fr" ? "et le dossier du pays concerné." : "and relevant country dossier."}</p></div><div className="mt-5"><Link href="/best/best-vpn" className="inline-flex items-center gap-2 rounded-lg border px-4 py-2 font-semibold hover:bg-muted">{copy.comparePaid} <ArrowRight className="size-4" aria-hidden="true" /></Link></div></div></div></section>
