...
import {
  freeVpnCopy,
  spanishFreeVpnCopy,
  type FreeVpnCardCopy,
  type FreeVpnCopy,
} from "@/components/editorial/free-vpn-copy";
...
function FreeTierCard({
  ...
  officialUrl,
  copy,
}: FreeVpnCardCopy & { rank: number; copy: FreeVpnCopy }) {
  return (
    <article className="rounded-2xl border bg-card p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">
            {rank}. {badge ?? "Free tier"}
          </p>
      ...
      <dl className="mt-5 grid gap-3 text-sm">
        <div className="rounded-lg bg-muted/50 p-3">
          <dt className="font-semibold">{copy.cardBoundary}</dt>
          <dd className="mt-1 text-muted-foreground">{limit}</dd>
        </div>
        <div className="rounded-lg bg-muted/50 p-3">
          <dt className="font-semibold">{copy.cardFit}</dt>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <AffiliateButton
          vpnId={vpnId}
          vpnName={name}
          affiliateUrl={getVpnAffiliateUrl(vpnId)}
          size="sm"
        >
          {copy.cardVisit} {name}
        </AffiliateButton>
...
export function FreeVpnEditorialPage({
  ...
  const localizedPageUrl =
    locale === "en" ? pageUrl : `${siteUrl}/${locale}/best/free-vpn`;
  return (
    <>
      <ArticleJsonLd
        title={copy.jsonLdTitle}
      ...
      <BestVpnEditorialTemplate
        brief={editorialContentBriefs.freeVpn}
        navigation={copy.nav}
      >
        <article>
          <section className="border-b bg-gradient-to-br from-emerald-500/10 via-background to-background py-14 lg:py-20">
                ...
                <p className="mt-5 text-sm leading-6 text-muted-foreground">
                  ...
                  <Link href="/affiliate-disclosure" className="underline">
                    {locale === "fr"
                      ...
                      : locale === "es"
                        ? "leer nuestra declaración"
                        : "read our disclosure"}
                  </Link>
                  .{" "}
                  {locale === "fr"
                    ? "Les forfaits et politiques des fournisseurs peuvent changer."
                    : locale === "es"
                      ? "Los planes y las políticas de los proveedores pueden cambiar."
                      : "Provider plans and policies can change."}
                </p>
              </div>
            </div>
          </section>
          ...
          <section
            id="quick-answer"
            className="container max-w-5xl scroll-mt-24 py-10 lg:py-14"
          >
            <div className="rounded-2xl border border-emerald-300 bg-emerald-50 p-6 text-emerald-950">
              <h2 className="text-2xl font-bold">{copy.quickAnswer}</h2>
              <p className="mt-3 max-w-3xl leading-7">{copy.quickAnswerText}</p>
            </div>
          </section>
          ...
          >
            <div className="max-w-3xl">
              ...
              <h2 className="mt-3 text-3xl font-bold">{copy.freeTiersTitle}</h2>
          ...
          >
            <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
              <div>
                ...
                <h2 className="mt-3 text-3xl font-bold">{copy.paidTitle}</h2>
                <p className="mt-4 leading-7 text-muted-foreground">
                  {copy.paidIntro}
                </p>
              </div>
              <div className="rounded-xl border border-amber-300 bg-amber-50 p-5 text-sm leading-6 text-amber-950">
                <div className="flex gap-3">
                  ...
                  />
                  <p>
                    <strong>{copy.restrictedLead}</strong> {copy.restrictedBody}{" "}
                    <Link
                      href="/guides/vpn-for-restricted-networks"
                      className="underline"
                    >
                      {locale === "fr"
                        ? "guide des réseaux restreints"
                        : locale === "es"
                          ? "guía de redes restringidas"
                          : "restricted-network guide"}
                    </Link>
                    ,{" "}
                    <Link
                    ...
                    >
                      {locale === "fr"
                        ? "guide de l’obfuscation"
                        : locale === "es"
                          ? "guía de ofuscación"
                          : "obfuscation guide"}
                    </Link>{" "}
                    {locale === "fr"
                      ? "et le dossier du pays concerné."
                      : locale === "es"
                        ? "y el dossier del país correspondiente."
                        : "and relevant country dossier."}
                  </p>
                </div>
                <div className="mt-5">
                  <Link
                    href="/best/best-vpn"
                    className="inline-flex items-center gap-2 rounded-lg border px-4 py-2 font-semibold hover:bg-muted"
                  >
                    {copy.comparePaid}{" "}
          ...
          >
            <h2 className="text-3xl font-bold">{copy.sourcesTitle}</h2>
            <ul className="mt-5 list-disc space-y-2 pl-5 text-sm leading-6 text-muted-foreground">
              <li>
                <a
                  className="underline"
                  href="https://protonvpn.com/free-vpn/download"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  {copy.sourceProton}
              ...
              <li>
                <a
                  ...
                  rel="noopener noreferrer"
                >
                  {copy.sourceTunnelBear}
                </a>
              </li>
              <li>{copy.sourceDataForSeo}</li>
            </ul>
            <p className="mt-6 text-sm leading-6 text-muted-foreground">
              ...
              ,{" "}
              <Link href="/guides/public-wifi-safety" className="underline">
                {copy.continueWifi}
              </Link>
              ,{" "}
              <Link
                ...
                className="underline"
              >
                {copy.continueProtocol}
              </Link>{" "}
              {locale === "fr" ? "ou le" : locale === "es" ? "o la" : "or the"}{" "}
